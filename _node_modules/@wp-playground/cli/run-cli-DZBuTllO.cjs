"use strict";const m=require("@php-wasm/logger"),b=require("@php-wasm/universal"),R=require("@wp-playground/blueprints"),U=require("@wp-playground/common"),c=require("fs"),q=require("worker_threads"),Se=require("@php-wasm/node"),d=require("path"),pe=require("child_process"),xe=require("util"),me=require("express"),Ee=require("stream"),Te=require("stream/promises"),$e=require("yargs"),N=require("@wp-playground/storage"),se=require("@php-wasm/progress"),Ie=require("@wp-playground/wordpress"),M=require("fs-extra"),ke=require("module"),ee=require("os"),Ce=require("@php-wasm/xdebug-bridge"),ie=require("tmp-promise"),L=require("@php-wasm/cli-util"),Re=require("crypto"),F=require("@wp-playground/tools"),ae=require("wasm-feature-detect");var k=typeof document<"u"?document.currentScript:null;function X(e){const t=[];for(const r of e){const o=r.split(":");if(o.length!==2)throw new Error(`Invalid mount format: ${r}.
				Expected format: /host/path:/vfs/path.
				If your path contains a colon, e.g. C:\\myplugin, use the --mount-dir option instead.
				Example: --mount-dir C:\\my-plugin /wordpress/wp-content/plugins/my-plugin`);const[n,s]=o;if(!c.existsSync(n))throw new Error(`Host path does not exist: ${n}`);t.push({hostPath:n,vfsPath:s})}return t}function le(e){if(e.length%2!==0)throw new Error("Invalid mount format. Expected: /host/path /vfs/path");const t=[];for(let r=0;r<e.length;r+=2){const o=e[r],n=e[r+1];if(!c.existsSync(o))throw new Error(`Host path does not exist: ${o}`);t.push({hostPath:d.resolve(process.cwd(),o),vfsPath:n})}return t}async function Me(e,t){for(const r of t)await e.mount(r.vfsPath,Se.createNodeFsMountHandler(r.hostPath))}const ue={step:"runPHP",code:{filename:"activate-theme.php",content:`<?php
...
`+a),process.exit(1)),console.error(a),process.exit(1)}).strictOptions().check(async a=>{if(a["skip-wordpress-install"]===!0&&(a["wordpress-install-mode"]="do-not-attempt-installing",a.wordpressInstallMode="do-not-attempt-installing"),a.wp!==void 0&&typeof a.wp=="string"&&!ze(a.wp))try{new URL(a.wp)}catch{throw new Error('Unrecognized WordPress version. Please use "latest", "beta", "trunk", "nightly", a URL, or a numeric version such as "6.2", "6.0.1", "6.2-beta1", or "6.2-RC1" (see --help for details).')}const T=a["site-url"];if(typeof T=="string"&&T.trim()!=="")try{new URL(T)}catch{throw new Error(`Invalid site-url "${T}". Please provide a valid URL (e.g., http://localhost:8080 or https://example.com)`)}if(a["auto-mount"]){let v=!1;try{v=c.statSync(a["auto-mount"]).isDirectory()}catch{v=!1}if(!v)throw new Error(`The specified --auto-mount path is not a directory: '${a["auto-mount"]}'.`)}if(a["experimental-blueprints-v2-runner"]===!0)throw new Error("Blueprints v2 are temporarily disabled while we rework their runtime implementation.");if(a.mode!==void 0)throw new Error("The --mode option requires the --experimentalBlueprintsV2Runner flag.");return!0});s.wrap(s.terminalWidth());const i=await s.argv,l=i._[0];["start","run-blueprint","server","build-snapshot","php"].includes(l)||(s.showHelp(),process.exit(1));const u=i.define||{},w=i["define-bool"]||{},A=i["define-number"]||{},p=a=>a in u||a in w||a in A;p("WP_DEBUG")||(u.WP_DEBUG="true"),p("WP_DEBUG_LOG")||(u.WP_DEBUG_LOG="true"),p("WP_DEBUG_DISPLAY")||(u.WP_DEBUG_DISPLAY="false");const E={...i,define:u,command:l,mount:[...i.mount||[],...i["mount-dir"]||[]],"mount-before-install":[...i["mount-before-install"]||[],...i["mount-dir-before-install"]||[]]},y=await te(E);y===void 0&&process.exit(0);const I=(()=>{let a;return async()=>{a===void 0&&(a=y[Symbol.asyncDispose]()),await a,process.exit(0)}})();return process.on("SIGINT",I),process.on("SIGTERM",I),{[Symbol.asyncDispose]:async()=>{process.off("SIGINT",I),process.off("SIGTERM",I),await y[Symbol.asyncDispose]()},[Y]:{cliServer:y}}}catch(t){console.error(t);const r=process.argv.includes("--debug");if(t instanceof Error)if(r)b.printDebugDetails(t);else{const o=[];let n=t;do o.push(n.message),n=n.cause;while(n instanceof Error);console.error("\x1B[1m"+o.join(" caused by: ")+"\x1B[0m")}else console.error("\x1B[1m"+be(t)+"\x1B[0m");process.exit(1)}}function be(e){if(e instanceof Error)return e.message;if(e&&typeof e=="object"){const t=[],r=e;if(r.name&&t.push(String(r.name)),r.message&&t.push(String(r.message)),r.errno!==void 0&&t.push(`errno: ${r.errno}`),t.length>0)return t.join(" — ");try{return JSON.stringify(e)}catch{return String(e)}}return String(e)}function de(e,t){return e.find(r=>r.vfsPath.replace(/\/$/,"")===t.replace(/\/$/,""))}const Y=Symbol("playground-cli-testing"),B=e=>process.stdout.isTTY?"\x1B[1m"+e+"\x1B[0m":e,it=e=>process.stdout.isTTY?"\x1B[31m"+e+"\x1B[0m":e,ce=e=>process.stdout.isTTY?`\x1B[2m${e}\x1B[0m`:e,O=e=>process.stdout.isTTY?`\x1B[3m${e}\x1B[0m`:e,Z=e=>process.stdout.isTTY?`\x1B[33m${e}\x1B[0m`:e;async function te(e){let t;const r=e.internalCookieStore?new b.HttpCookieStore:void 0,o=[],n=new Map;if(e.command==="start"&&(e=at(e)),e.autoMount!==void 0&&(e.autoMount===""&&(e={...e,autoMount:process.cwd()}),e=he(e)),e.wordpressInstallMode===void 0&&(e.wordpressInstallMode="download-and-install"),e.quiet&&(e.verbosity="quiet",delete e.quiet),e.debug&&(e.verbosity="debug",delete e.debug),e.verbosity){const p=Object.values(V).find(E=>E.name===e.verbosity).severity;m.logger.setSeverityFilterLevel(p)}e.intl||(e.intl=!0),e.redis===void 0&&(e.redis=await ae.jspi()),e.memcached===void 0&&(e.memcached=await ae.jspi()),e.phpmyadmin&&(e.phpmyadmin===!0&&(e.phpmyadmin="/phpmyadmin"),e.pathAliases=[{urlPrefix:e.phpmyadmin,fsPath:F.PHPMYADMIN_INSTALL_PATH}]);const s=new st({verbosity:e.verbosity||"normal"});e.command==="server"&&(s.printBanner(),s.printConfig({phpVersion:e.php||U.RecommendedPHPVersion,wpVersion:e.wp||"latest",port:e.port??9400,xdebug:!!e.xdebug,intl:!!e.intl,redis:!!e.redis,memcached:!!e.memcached,mounts:[...e.mount||[],...e["mount-before-install"]||[]],blueprint:typeof e.blueprint=="string"?e.blueprint:void 0}));const i=e.command==="server"?e.port??9400:0,l=new b.FileLockManagerInMemory;let u=!1,w=!0;const A=await Oe({port:e.port?e.port:await Fe(i)?0:i,onBind:async(p,E)=>{const y="127.0.0.1",I=`http://${y}:${E}`,a=e["site-url"]||I,T=6,v="-playground-cli-site-",C=await et(v);m.logger.debug(`Native temp dir for VFS root: ${C.path}`);const W="WP Playground CLI - Listen for Xdebug",oe=".playground-xdebug-root",z=d.join(process.cwd(),oe);if(await L.removeTempDirSymlink(z),e.xdebug){const h={hostPath:d.join(".",d.sep,oe),vfsPath:"/"};if(b.SupportedPHPVersions.indexOf(e.php||U.RecommendedPHPVersion)<=b.SupportedPHPVersions.indexOf("8.5"))await L.createTempDirSymlink(C.path,z,process.platform),e.xdebug=L.makeXdebugConfig({cwd:process.cwd(),mounts:[h,...e["mount-before-install"]||[],...e.mount||[]],pathSkippings:["/dev/","/home/","/internal/","/request/","/proc/"]}),console.log(B("Xdebug configured successfully")),console.log(Z("Playground source root: ")+".playground-xdebug-root"+O(ce(" – you can set breakpoints and preview Playground's VFS structure in there.")));else if(e.experimentalUnsafeIdeIntegration){await L.createTempDirSymlink(C.path,z,process.platform);try{await L.clearXdebugIDEConfig(W,process.cwd());const f=typeof e.xdebug=="object"?e.xdebug:{},g=await L.addXdebugIDEConfig({name:W,host:y,port:E,ides:e.experimentalUnsafeIdeIntegration,cwd:process.cwd(),mounts:[h,...e["mount-before-install"]||[],...e.mount||[]],ideKey:f.ideKey||"WPPLAYGROUNDCLI"}),S=e.experimentalUnsafeIdeIntegration,x=S.includes("vscode"),$=S.includes("phpstorm"),H=Object.values(g);console.log(""),H.length>0?(console.log(B("Xdebug configured successfully")),console.log(Z("Updated IDE config: ")+H.join(" ")),console.log(Z("Playground source root: ")+".playground-xdebug-root"+O(ce(" – you can set breakpoints and preview Playground's VFS structure in there.")))):(console.log(B("Xdebug configuration failed.")),console.log("No IDE-specific project settings directory was found in the current working directory.")),console.log(""),x&&g.vscode&&(console.log(B("VS Code / Cursor instructions:")),console.log("  1. Ensure you have installed an IDE extension for PHP Debugging"),console.log(`     (The ${B("PHP Debug")} extension by ${B("Xdebug")} has been a solid option)`),console.log("  2. Open the Run and Debug panel on the left sidebar"),console.log(`  3. Select "${O(W)}" from the dropdown`),console.log('  3. Click "start debugging"'),console.log("  5. Set a breakpoint. For example, in .playground-xdebug-root/wordpress/index.php"),console.log("  6. Visit Playground in your browser to hit the breakpoint"),$&&console.log("")),$&&g.phpstorm&&(console.log(B("PhpStorm instructions:")),console.log(`  1. Choose "${O(W)}" debug configuration in the toolbar`),console.log("  2. Click the debug button (bug icon)`"),console.log("  3. Set a breakpoint. For example, in .playground-xdebug-root/wordpress/index.php"),console.log("  4. Visit Playground in your browser to hit the breakpoint")),console.log("")}catch(f){throw new Error("Could not configure Xdebug",{cause:f})}}}const ge=d.dirname(C.path),Pe=2*24*60*60*1e3;tt(v,Pe,ge);const ne=d.join(C.path,"internal");c.mkdirSync(ne);const ve=["wordpress","tools","tmp","home"];for(const h of ve){const P=g=>g.vfsPath===`/${h}`;if(!(e["mount-before-install"]?.some(P)||e.mount?.some(P))){const g=d.join(C.path,h);c.mkdirSync(g),e["mount-before-install"]===void 0&&(e["mount-before-install"]=[]),e["mount-before-install"].unshift({vfsPath:`/${h}`,hostPath:g})}}if(e["mount-before-install"])for(const h of e["mount-before-install"])m.logger.debug(`Mount before WP install: ${h.vfsPath} -> ${h.hostPath}`);if(e.mount)for(const h of e.mount)m.logger.debug(`Mount after WP install: ${h.vfsPath} -> ${h.hostPath}`);let D;e["experimental-blueprints-v2-runner"]?D=new Qe(e,{siteUrl:a,cliOutput:s}):(D=new Ke(e,{siteUrl:a,cliOutput:s}),typeof e.blueprint=="string"&&(e.blueprint=await Ge({sourceString:e.blueprint,blueprintMayReadAdjacentFiles:e["blueprint-may-read-adjacent-files"]===!0})));let G=!1;const _=async function(){G||(G=!0,await Promise.all(o.map(async P=>{await n.get(P)?.dispose(),await P.worker.terminate()})),p&&await new Promise(P=>{p.close(P),p.closeAllConnections()}),await C.cleanup())};try{const h=[],P=D.getWorkerType();for(let f=0;f<T;f++){const g=re(P,{onExit:S=>{G||S!==0&&m.logger.error(`Worker ${f} exited with code ${S}
`)}}).then(async S=>{o.push(S);const x=await lt(l),$=await D.bootRequestHandler({worker:S,fileLockManagerPort:x,nativeInternalDirPath:ne});return n.set(S,$),[S,$]});h.push(g),f===0&&await g}await Promise.all(h),t=b.createObjectPoolProxy(o.map(f=>n.get(f)));{const f=new q.MessageChannel,g=f.port1,S=f.port2;if(await b.exposeAPI({applyPostInstallMountsToAllWorkers:async()=>{for(const x of n.values())await x.mountAfterWordPressInstall(e.mount||[])}},void 0,g),await D.bootWordPress(t,S),g.close(),u=!0,!e["experimental-blueprints-v2-runner"]){const x=await D.compileInputBlueprint(e["additional-blueprint-steps"]||[]);x&&await R.runBlueprintV1Steps(x,t)}if(e.phpmyadmin&&!await t.fileExists(`${F.PHPMYADMIN_INSTALL_PATH}/index.php`)){const x=await F.getPhpMyAdminInstallSteps(),$=await R.compileBlueprintV1({steps:x});await R.runBlueprintV1Steps($,t)}if(e.command==="build-snapshot"){await dt(t,e.outfile),s.printStatus(`Exported to ${e.outfile}`),await _();return}else if(e.command==="run-blueprint"){s.finishProgress("Done"),await _();return}else if(e.command==="php"){const x=["/internal/shared/bin/php",...(e._||[]).slice(1)],$=await t.cli(x),[H]=await Promise.all([$.exitCode,$.stdout.pipeTo(new WritableStream({write(Q){process.stdout.write(Q)}})),$.stderr.pipeTo(new WritableStream({write(Q){process.stderr.write(Q)}}))]);await _(),process.exit(H)}}if(s.finishProgress(),s.printReady(I,T),e.phpmyadmin){const f=d.join(e.phpmyadmin,F.PHPMYADMIN_ENTRY_PATH);s.printPhpMyAdminUrl(new URL(f,I).toString())}return e.xdebug&&e.experimentalDevtools&&(await Ce.startBridge({phpInstance:t,phpRoot:"/wordpress"})).start(),{playground:t,server:p,serverUrl:I,[Symbol.asyncDispose]:_,[Y]:{workerThreadCount:T}}}catch(h){if(e.verbosity!=="debug")throw h;let P="";throw await t?.fileExists(m.errorLogPath)&&(P=await t.readFileAsText(m.errorLogPath)),await _(),new Error(P,{cause:h})}},async handleRequest(p){if(!u)return b.StreamedPHPResponse.forHttpCode(502,"WordPress is not ready yet");if(w){w=!1;const y={"Content-Type":["text/plain"],"Content-Length":["0"],Location:[p.url]};return p.headers?.cookie?.includes("playground_auto_login_already_happened")&&(y["Set-Cookie"]=["playground_auto_login_already_happened=1; Max-Age=0; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Path=/"]),b.StreamedPHPResponse.fromPHPResponse(new b.PHPResponse(302,y,new Uint8Array))}r&&(p={...p,headers:{...p.headers,cookie:r.getCookieRequestHeader()}});const E=await t.requestStreamed(p);if(r){const y=await E.headers;r.rememberCookiesFromResponseHeaders(y),delete y["set-cookie"]}return E}}).catch(p=>{s.printError(be(p)),process.exit(1)});return A&&e.command==="start"&&!e.skipBrowser&&ut(A.serverUrl),A}function at(e){let t={...e,command:"server"};e.noAutoMount||(t.autoMount=d.resolve(process.cwd(),t.path??""),t=he(t),delete t.autoMount);const r=de(t["mount-before-install"]||[],"/wordpress")||de(t.mount||[],"/wordpress");if(r)console.log("Site files stored at:",r?.hostPath),e.reset&&(console.log(""),console.log(it("This site is not managed by Playground CLI and cannot be reset.")),console.log("(It's not stored in the ~/.wordpress-playground/sites/<site-id> directory.)"),console.log(""),console.log("You may still remove the site's directory manually if you wish."),process.exit(1));else{const o=t.autoMount||process.cwd(),n=Re.createHash("sha256").update(o).digest("hex"),s=ee.homedir(),i=d.join(s,".wordpress-playground/sites",n);console.log("Site files stored at:",i),c.existsSync(i)&&e.reset&&(console.log("Resetting site..."),c.rmdirSync(i,{recursive:!0})),c.mkdirSync(i,{recursive:!0}),t["mount-before-install"]=[...t["mount-before-install"]||[],{vfsPath:"/wordpress",hostPath:i}],t.wordpressInstallMode=c.readdirSync(i).length===0?"download-and-install":"install-from-existing-files-if-needed"}return t}const J=new b.ProcessIdAllocator;function re(e,{onExit:t}={}){let r;return e==="v1"?r=new q.Worker(new URL("./worker-thread-v1.cjs",typeof document>"u"?require("url").pathToFileURL(__filename).href:k&&k.tagName.toUpperCase()==="SCRIPT"&&k.src||new URL("run-cli-DZBuTllO.cjs",document.baseURI).href)):r=new q.Worker(new URL("./worker-thread-v2.cjs",typeof document>"u"?require("url").pathToFileURL(__filename).href:k&&k.tagName.toUpperCase()==="SCRIPT"&&k.src||new URL("run-cli-DZBuTllO.cjs",document.baseURI).href)),new Promise((o,n)=>{const s=J.claim();r.once("message",function(l){l.command==="worker-script-initialized"&&o({processId:s,worker:r,phpPort:l.phpPort})}),r.once("error",function(l){J.release(s),console.error(l);const u=l?.message||(l?String(l):"unknown error"),w=new Error(`Worker failed to load. Original error: ${u}`,{cause:l});n(w)});let i=!1;r.once("spawn",()=>{i=!0}),r.once("exit",l=>{J.release(s),i||n(new Error(`Worker exited before spawning: ${l}`)),t?.(l)})})}async function lt(e){const{port1:t,port2:r}=new q.MessageChannel;return await b.exposeSyncAPI(e,t),r}function ut(e){const t=ee.platform();let r;switch(t){case"darwin":r=`open "${e}"`;break;case"win32":r=`start "" "${e}"`;break;default:r=`xdg-open "${e}"`;break}pe.exec(r,o=>{o&&m.logger.debug(`Could not open browser: ${o.message}`)})}async function dt(e,t){await e.run({code:`<?php
		$zip = new ZipArchive();
...
//# sourceMappingURL=run-cli-DZBuTllO.cjs.map
